...
class RQuant_Stable:
    ...
    def gen_stable(self):
        ...
        for i in range(self.ndays):
            U = np.pi * np.random.random_sample() - np.pi / 2.0
            W = np.random.exponential(1)
            X = (
                math.sin(1.7 * U)
                / (math.cos(U)) ** (1.0 / 1.7)
                * (math.cos(U - 1.7 * U) / W) ** (-0.7 / 1.7)
            )
            Y = X + 1.0
            data[i] = Y
        return data
    ...
    def get_n_quantile_p_while(self, quantile=0.01, delta=0.01):
        ...
        while True:
            ...
            if res.shape[0] > 10 and data_mean[-10:].std() < np.fabs(
                delta * res.mean()
            ):
                break
        return res
...
def main(nevent, delta):
    ...
    data1_range = [
        np.quantile(generator.data1, 0.01),
        np.quantile(generator.data1, 0.99),
    ]
    plot_histogram(
        ax[0, 0],
        generator.data1,
        bins=100,
        range=data1_range,
        title="stable distribution of return1",
        histtype="step",
    )
    ...
    data10_range = [
        np.quantile(generator.data10, 0.01),
        np.quantile(generator.data10, 0.99),
    ]
    plot_histogram(
        ax[0, 1],
        generator.data10,
        bins=500,
        range=data10_range,
        title="distribution of return10",
        log=True,
    )
    ...
    plot_histogram(
        ...
        bins=100,
        range=q_limit_range,
        title="1% quantile spectrum in [min,max] range",
        log=True,
        histtype="step",
        density=True,
        label=f"n events = {data_q.shape[0]}",
    )
    ...
    plot_histogram(
        ...
        bins=100,
        range=(q, data_q_limit.max()),
        title="1% quantile spectrum in [1% quantile,max] range",
        log=True,
        histtype="step",
        density=True,
        label=f"n events = {data_q.shape[0]}",
    ...
    print(
        f"min/max of generated spectrum: {np.round(data_q.min())} {np.round(data_q.max())}"
    )
    print(f"Number of events = {data_q.shape[0]}")
    print(
        "Kolmogorov-Smirnov test comparing generated spectrum and high statistics spectrum:"
    )
